...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  const thumbnail = product.imageResponses?.find(img => img.isThumbnail) 
    || product.imageResponses?.[0] 
    || { imageUrl: 'https://placehold.co/400x400?text=No+Image' }; // Fallback
  ...
  // Mocking "Đã bán" count for UI purposes
  const mockSoldCount = Math.floor(Math.random() * 500) + 10;
