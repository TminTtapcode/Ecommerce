...
const STATUS_BADGE: Record<string, { label: string; className: string }> = {
  ...
  CONFIRMED: { label: 'Chờ xử lý', className: 'bg-blue-100 text-blue-700' },
...
export const VendorOrderListPage = () => {
  ...
  const handleUpdateStatus = async (orderId: number, newStatus: 'CONFIRMED' | 'SHIPPED' | 'DELIVERED' | 'CANCELLED', reason?: string) => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-8 px-4">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Kênh Người Bán — Quản lý Đơn hàng</h1>
        <p className="text-sm text-gray-500 mt-1">Xem và xử lý đơn hàng của Shop bạn</p>
      </div>
                  ...
                  <div className="space-y-2">
                    {order.items.map((item) => (
                      <div key={item.id} className="flex justify-between items-center text-sm">
                        ...
                        <span className="text-gray-600 font-medium">{item.subTotal.toLocaleString('vi-VN')}₫</span>
                      </div>
                    ))}
                  </div>
