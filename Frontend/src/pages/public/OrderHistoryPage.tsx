                        ...
                        <div key={order.id} className="bg-white rounded-sm shadow-sm overflow-hidden">
                            ...
                            <div className="p-4 border-b border-gray-100">
                                {order.items.map(item => (
                                    <div key={item.id} className="flex gap-4 py-3 first:pt-0 last:pb-0">
                                        <div className="w-20 h-20 bg-gray-100 border rounded flex items-center justify-center text-gray-400 shrink-0">
                                            {/* Placeholder for image */}
                                            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        </div>
                                        <div className="flex-1">
                                            <Link to={`/products/${item.productVariantId}`} className="text-gray-800 hover:text-orange-500 text-base">{item.productName}</Link>
                            ...
                            <div className="p-4 bg-orange-50/30 flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <Link to={`/orders/${order.id}`} className="px-6 py-2 bg-orange-500 text-white rounded hover:bg-orange-600 transition-colors text-sm">
                                        Xem Chi Tiết
                                    </Link>
                                </div>
                            </div>
